...
/** Detect if the current URL is the email-verification callback. */
function isVerifyRoute() {
  return window.location.pathname === '/verify-email' && window.location.search.includes('token=')
}
...
function AppInner() {
  ...
  const [verifyPage, setVerifyPage] = useState(isVerifyRoute)
  ...
  if (verifyPage) {
    return <EmailVerifyPage onVerified={() => setVerifyPage(false)} />
  }
